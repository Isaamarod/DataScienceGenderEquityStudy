...
```{r pressure, echo=FALSE}
by_getting_married_2009_2018 <- gender %>%
  select(`GETTING MARRIED`, region, year, global_index) %>%
  filter(year == 2009 | year == 2018) %>%
  group_by(region, year) %>%
  summarize(meanGetMarried = mean(`GETTING MARRIED`))

head(by_getting_married_2009_2018)
...
```{r}
by_continent_2009_2018_OECD <- gender %>%
  select(region, year, country, global_index) %>%
  filter((year == 2009 | year == 2018) & gender$region == 'High income: OECD' ) %>%
  group_by(region, year, country) %>%
  summarize(meanGlobal = mean(global_index))

head(by_continent_2009_2018_OECD)

ggplot(by_continent_2009_2018_OECD, aes(x=country,y=meanGlobal,fill=factor(year)))+
  geom_bar(stat="identity",position="dodge")+
  geom_text(aes(label= round(meanGlobal, digits = 2)), position=position_dodge(width=0.9), vjust=-0.25)+
  scale_fill_discrete(name="Año",
                      breaks=c(1, 2),
